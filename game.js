(() => {
  ...
  function randomPoint() { let point; do { point = { x:Math.floor(Math.random()*cols), y:Math.floor(Math.random()*rows) }; } while (occupied(point)); return point; }
  ...
  function resetState() { clearTimers(); const center = {x:Math.floor(cols/2),y:Math.floor(rows/2)}; snake = [center,{x:center.x-1,y:center.y},{x:center.x-2,y:center.y}]; direction = directions.right; nextDirection = direction; score = 0; updateScore(); enemies = []; for (let i=0;i<enemiesByDifficulty[difficultySelect.value];i+=1) enemies.push(randomPoint()); food = randomPoint(); running = false; paused = false; pauseButton.disabled = true; pauseButton.textContent = "Pause"; gameOverPanel.hidden = true; draw(); }
